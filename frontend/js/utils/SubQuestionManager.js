...
export default class SubQuestionManager {
  constructor(store, allSteps, sources = {}) {
    this.store = store;
  ...
  // Crée ou récupère l'instance d'une sous-question
  initSubQuestion(optionValue, subStep) {
    if (!this.subQuestionInstances[optionValue]) {
      ...
      instance.parentValue = optionValue;
      this.subQuestionInstances[optionValue] = instance;
    }
    return this.subQuestionInstances[optionValue];
  }
    ...
    Object.values(this.subQuestionInstances).forEach(subInstance => {
      ...
      if (!comp.setAnswerOriginal) {
        ...
        comp.setAnswer = (val) => {
          ...
          this.onSubAnswerChange(subInstance.parentValue, val);
        };
      }
    });
  }

  // Méthode à implémenter dans la classe enfant pour synchroniser la réponse principale
  onSubAnswerChange(optionValue, val) { }
}
